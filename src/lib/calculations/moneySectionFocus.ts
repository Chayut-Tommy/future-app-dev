 ...
 * targets ('aup' | 'timeline') are a different domain from Grow's, and
 * sectionFocus.ts's SectionFocusMeasurements/SectionFocusFulfillment types
 * are already a closed, specific shape for Grow's own seven targets; adding
 * an unrelated screen's targets there would blur what that module is for.
 * The PATTERN is reused byte-for-byte; the types are not force-shared.
 *
 ...
 */
export type MoneySectionFocusTarget = 'aup' | 'timeline';
 ...
 */
export function parseMoneySectionFocusRequest(scrollTo: unknown, requestId: unknown): MoneySectionFocusRequest | null {
  const VALID_TARGETS: MoneySectionFocusTarget[] = ['aup', 'timeline'];
  if (typeof scrollTo !== 'string' || !(VALID_TARGETS as string[]).includes(scrollTo)) return null;
  if (typeof requestId !== 'number') return null;
  return { target: scrollTo as MoneySectionFocusTarget, requestId };
}
