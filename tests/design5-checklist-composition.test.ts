...
console.log('\n=== 8. B9 — canonical Save-feedback carry-over closure ===');
{
  ...
  assert('8d. standalone card saves route through the boundary BEFORE the debt-reduced celebration (so the celebration claims the plain toast)', /if \(!embedded\) confirmSaveSuccess\(buildSaveConfirmation\('Credit card', editCard \? 'updated' : 'added'\)\);\s*\n\s*if \(editCard && payload\.currentBalance < editCard\.currentBalance\) celebrate\(/.test(code(read('src/components/credit/AddCreditCardModal.tsx'))));
