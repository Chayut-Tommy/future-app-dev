...
console.log('\n=== 3. NOTHING financial was traded away for the consolidation ===');
{
  ...
  assert("3d. income Save is still unreachable from the mid-cycle step — the guard that stops a stray tap minting an unused midCycleRecurringItemId", /onCanSaveChange\?\.\(canSave && formStep === 'details' && !completion\.isPending\);/.test(INCOME) && /if \(!canSave \|\| !parsedIncome\.valid \|\| formStep !== 'details'\) return;/.test(INCOME));
