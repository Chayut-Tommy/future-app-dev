...
console.log('\n=== §A.4 Pay cycle progress: the 20 Sep Dividends +$1,000 gets an expected-income marker; AUP is unchanged ===');
{
  ...
  assert('the bill markers are identical with or without the expected-income stream, and no `income` (included) marker exists in AUP mode', JSON.stringify(before.markers.filter((m) => m.kind !== 'expected_income')) === JSON.stringify(rail.markers.filter((m) => m.kind !== 'expected_income')) && !rail.markers.some((m) => m.kind === 'income'));
