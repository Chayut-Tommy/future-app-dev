// A1 — classification SELECTION LIFECYCLE (part 3): double-press dedup (rendered).
// Isolated in its own file: the double-press leaves extra sheet slide-out timers
// that the documented heavy-suite cross-test timer race can turn into flakiness.
//
...
describe('A1 — classification selection lifecycle III: double-press dedup (rendered)', () => {
  beforeEach(async () => { await AsyncStorage.clear(); });

  test('§9 duplication — rapid double-press on a choice commits exactly ONCE', async () => {
    ...
    // Two rapid presses on the same row before dismissal completes — the OS
    // still fires a single completion, and the commit must happen exactly once.
    ...
    expect(screen.getByTestId('probe')).toHaveTextContent(/cba:1300/); // one balance effect only
    await drain();
  }, 60000);
});
