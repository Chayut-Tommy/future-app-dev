...
console.log('\n=== 3. Exactly four haptic events, one dispatch module ===');
{
  ...
  assert('3f2. every Add task funnels through the ONE shared save-success authority (handleSaveSuccessClose)', /function handleSaveSuccessClose\(\) \{\s*\n[\s\S]{0,900}?confirmSaveSuccess\(/.test(SHEET_HAPTIC) && (SHEET_HAPTIC.match(/onSaveSuccess=\{handleSaveSuccessClose\}/g) ?? []).length === 9 && (SHEET_HAPTIC.match(/confirmSaveSuccess\(/g) ?? []).length === 1);
