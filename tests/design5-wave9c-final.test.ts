...
console.log('\n=== 4. Correction B — completion before creation in Money ===');
{
  ...
  assert('4m. editing updates the SAME id, adding creates — the existing CRUD, untouched', /updateRecurringItem\(editItem\.id, payload, \{ setAsMainPayday: willSetMainPayday \}\)/.test(INCOME_MODAL) && /addRecurringItem\(payload, \{ setAsMainPayday: willSetMainPayday, id \}\)/.test(INCOME_MODAL)); // Pass D0 — Add carries the draft's one stable id // C.4: same CRUD, plus the atomic main-payday option
